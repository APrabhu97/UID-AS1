...
$(document).on('mousedown', 'td.sudoku-cell', function(ev) {
  ...
  const text = $(document).find("#sn"+pos)[0]?.innerText;
  let n = parseInt(text == "I" ? "1" : text);
  n= (isNaN(n) ? 1 : (n+1)%5);
  if(pos == currCell){
    renderCurrCell(pos, n);
  }
  currCell = pos;
});

function renderCurrCell(pos, currNum){
...
$(document).on('click', '#newGame', function(ev) {
  flippage(1);
});
...
$(document).on('click', '#clearbutton', function(ev) {
  hidepopups();
  var state = currentstate();
  $(document).trigger('log', ['clearBoard', {seed: state.seed}]);
...
$(document).on('mousedown touchstart', '#checkbutton', function(ev) {
  ...
  if (conflicts.length == 0) {
    ...
    status = countfilled(sofar) == Sudoku.S ? "Puzzle Solved" : "ok so far";
  } else {
    // Oops - there is some mistake.
    status = "mistake";
    showpopup('#errors');
  }
  $("#showStatus").attr("class", "noShowBlur");
  $("#showStatus").text(status);
  $(document).trigger('log', ['checkStatus', {seed: 100, status: status}]);
  ev.stopPropagation();
});
...
$(document).on('mouseup mouseleave touchend', '#checkbutton', function() {
  $("#showStatus").attr("class", "showBlur");
...
$(document).on("keyup", function (evt) {
  if(['1', '2', '3', '4'].includes(evt.originalEvent.key)){
    renderCurrCell(currCell, evt.originalEvent.key);
  }
});
